...
export default function App() {
  ...
  const [apiKey, setApiKey] = useState(() => localStorage.getItem("tr_api_key") || "");
  const [showKeyInput, setShowKeyInput] = useState(() => !localStorage.getItem("tr_api_key"));
  const saveKey = (k) => { setApiKey(k); localStorage.setItem("tr_api_key", k); if(k) setShowKeyInput(false); };
  ...
  return (
    <div style={{minHeight:"100vh",background:G.bg,fontFamily:"'DM Sans',sans-serif",color:G.text}}>
      ...
      {showKeyInput && (
        <div style={{background:"#fffbeb",borderBottom:`1.5px solid ${G.amberBorder}`,padding:"14px 28px",
          display:"flex",alignItems:"center",gap:14,flexWrap:"wrap"}}>
          <span style={{fontSize:16}}>🔑</span>
          <div style={{flex:1,minWidth:260}}>
            <p style={{fontSize:13,fontWeight:700,color:G.amber,marginBottom:3}}>Anthropic API Key נדרש</p>
            <p style={{fontSize:11.5,color:"#92400e"}}>מקבלים ב-<a href="https://console.anthropic.com" target="_blank" rel="noopener noreferrer" style={{color:G.blue}}>console.anthropic.com</a> ← API Keys. נשמר רק בדפדפן שלך.</p>
          </div>
          <div style={{display:"flex",gap:8,alignItems:"center",flex:"1 1 340px"}}>
            <input
              type="password"
              placeholder="sk-ant-api03-..."
              defaultValue={apiKey}
              onBlur={e=>saveKey(e.target.value.trim())}
              onKeyDown={e=>e.key==="Enter"&&saveKey(e.target.value.trim())}
              style={{flex:1,borderRadius:9,padding:"9px 13px",fontSize:13,fontFamily:"'DM Sans',sans-serif",
                border:`1.5px solid ${G.amberBorder}`,background:"#fff",color:G.text}}
            />
            <button className="btn" onClick={()=>setShowKeyInput(false)} disabled={!apiKey}
              style={{padding:"9px 16px",borderRadius:9,background:apiKey?G.amber:"#ccc",color:"#fff",
      ...
      {!showKeyInput && apiKey && (
        <div style={{background:"#f0faf5",borderBottom:`1px solid ${G.greenBorder}`,
          padding:"7px 28px",display:"flex",alignItems:"center",justifyContent:"space-between"}}>
          <span style={{fontSize:12,color:G.green,fontWeight:600}}>🔑 API Key מוגדר</span>
          <button className="btn" onClick={()=>setShowKeyInput(true)}
            style={{fontSize:11,color:G.muted,background:"transparent",padding:"3px 8px",
